...
namespace GoogleVR.HelloVR
{
    ...
    public class MyObjectController : MonoBehaviour
    {
        ...
        public bool automaticAction = false; //Se activa una accion si se observa el objeto
        private bool timerRunning = false;
        private bool isHolding = false; //Se esta cargando un objeto?
        ...
        {
            if (locoMotion != null)
                locoMotion.teleportPlayer(this.transform.position);
            else
                Debug.Log("LocoMotion is null");
        }
        ...
        public void carryObject()
        {
            if (isHolding == false)
            {
                locoMotion.assignObject(this.gameObject, inactiveMaterial);
                isHolding = true;
            }
        }
        ...
        {
            if (isHolding == false)
               this.transform.position =  locoMotion.teleportObject(this.transform.position);
        }
        */
        private void Update()
        {
            if (timerRunning)
            {
                ...
                if (interactionTimer > triggerInteractionTime && automaticAction == true)
                {
                    teleportPlayer();
                } //call interact
            }

            if (spin && rigidbody != null)
            {
                rigidbody.AddTorque(this.transform.up * 100 * -1f);
                Debug.Log("Spinning");
            }
          //  else
